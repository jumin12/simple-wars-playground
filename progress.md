...
## Mobile order state / Stats page / layer independence follow-up (2026-07-10)
- Current request: bring terrain and NATO units still closer to the supplied reference; make the mobile UI exact; remove Deselect; tap empty map to deselect only when Move, Waypoint, or Formation is not armed; repair those three mode selectors; replace More with a Stats page containing name editing and service record; remove the card/dock gap; keep city names visible when only the Cities layer is disabled.
- Diagnosis: mobile order mode starts permanently on Move, has no off state, and portrait CSS masks the active styling for Waypoint/Formation. Empty touch taps never clear selection, touch taps can append instead of replace selection, and city-name drawing is nested entirely inside the Cities-layer branch.
- Mobile order-state fix complete: modes now start unarmed, each button toggles on/off with `aria-pressed` and visible armed chrome, touch taps replace (not append) single selection, empty taps deselect only while no order mode is armed, and touch path-stroke starts only in Move mode. Playwright touch tests passed click-off deselection, Move targeting, two-point Waypoint orders, and two-unit Formation drag orders with no console errors.
- Stats-page conversion complete: removed the Deselect control and overview pencil, replaced More with Stats, added a dedicated second page headed by the full division name + edit control, kept the service record there, and added an Orders return control. Automated rename changed the selected unit and both page titles consistently; page navigation and visibility passed with no console errors.
- Card/layer chunk complete: portrait command card now meets the dock exactly (measured 0px gap at 430x932). City rendering now treats `cities` and `cityNames` independently; instrumented draw checks produced names=2/markers=0 for names-only and names=0/markers=2 for cities-only. No Deselect element remains.
- Visual v2 complete: earthier reference palette, denser paired relief contours + hachures, long curved macro ridges instead of circular terrain light spots, smoother connected terrain blobs, stronger water grain, and taller gradient-shaded NATO counters with larger face symbols/brackets. The card emblem now includes the two-bar echelon mark; the top resources share one bordered panel with the reference divider treatment.
- Final regression: required web-game client produced gameplay PNG + text state with no error artifact. Real-touch Playwright covered unarmed tap-off, armed Move, two-point Waypoint, and drag Formation; all modes toggle back to null. Stats rename/navigation, zero-gap card geometry, names-only/cities-only layer drawing, 430x932 and 390x844 overflow, title fit, and `render_game_to_text.ui` all passed with no console errors.
